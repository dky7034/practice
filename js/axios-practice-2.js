...
async function getProductReviewsAvgRatingByid(id) {
  const data = (await axios.get(`${BASE_URL}/products/${id}`))["data"];
...
// getProductReviewsAvgRatingByid(10);
/* 출력 결과
3.3333333333333335
*/
...
async function getAllPostsTags() {
  const { data } = await axios(`${BASE_URL}/posts/tags`);
  const slugList = data.map((e) => {
    return e.slug;
  });
...
async function getProductsCountByKeyword(keyword) {
  const { data } = await axios.get(`${BASE_URL}/products/search`, {
    params: { q: keyword },
  });
  console.log(data["products"].length);
}
getProductsCountByKeyword("glass");
/* 출력 결과
// phone 검색 시
23
