...
async function getProductsFromOneCart(id) {
  const res = await axios.get(`${BASE_URL}/carts/${id}`);
  const products = res["data"]["products"];
  console.log(products);
}
// getProductsFromOneCart(10);
/* 출력 결과
[
  {
    id: 190,
...
// 4. 상품의 이름만 모은 배열 출력
async function getTitleFromGetaSingleCart(id) {
  const cart = await getaSingleCart(id);
